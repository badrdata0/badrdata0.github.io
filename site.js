...
function afficherAccueil() {
  ...
  document.getElementById("categories").innerHTML = CATEGORIES.map(c =>
    `<li><a href="categorie.html?c=${encodeURIComponent(c.id)}">${echap(c.titre)}</a></li>`).join("");
}
...
function afficherCategorie() {
  ...
  zone.innerHTML = `
    <a class="retour" href="index.html">‹ Essays</a>
    <h1 class="titre-cat">${echap(cat.titre)}</h1>
    <p class="vide">${echap(cat.description || "")}</p><hr>${liste}`;
}
